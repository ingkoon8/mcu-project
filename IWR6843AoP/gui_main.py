...
#
    def setConnectionLayout(self):
        ...
        self.configType.addItems(["3D People Counting", "SDK Out of Box Demo", "Long Range People Detection", "Sense and Detect HVAC Control", "Small Obstacle Detection", "(Legacy) 2D People Counting", "(Legacy): Overhead People Counting", "Capon3DAOP", "Replay"])
        self.configType.currentIndexChanged.connect(self.updateConfigType)
    ...
    def updateConfigType(self):
        print('configType Changed to: ' + self.configType.currentText())

        if (self.configType.currentText() == 'Small Obstacle Detection'):
            
            # Lock plot tracks off
            self.plotTracks.setChecked(False)
            self.plotTracks.setDisabled(True)

            # Lock display static points off
            self.staticclutter.setChecked(False)
            self.staticclutter.setDisabled(True)

            # Lock orientation in side mount
            self.orientationSelection.setCurrentIndex(0)
            self.orientationSelection.setDisabled(True)

            # Lock boundary box config
            ...
            # Unlock display static points
            self.staticclutter.setDisabled(False)

            # Unlock orientation
            self.orientationSelection.setDisabled(False)
    ...
    def drawBoundaryBox3d(self, index):
        ...
        # Color boundary boxes differently for small Obstacle detection
        if (self.configType.currentText() == 'Small Obstacle Detection'):
            self.boundaryBoxViz[index].setData(pos=boxLines,color=pg.glColor('g'),width=3,antialias=True,mode='lines')
            self.bottomSquare[index].setData(pos=squareLine,color=pg.glColor('g'),width=3,antialias=True,mode='line_strip')
        else:
    ...
    def updateGraph(self, parsedData):
        ...
        if (self.configType.currentText() == 'Small Obstacle Detection'):
            boundStates = parsedData[8]
            numZones = min(len(boundStates), len(self.boundaryBoxes))
            for i in range(numZones):
                if (boundStates[i]):
                    # Make first box turn red, rest yellow
                    if (i == 0):
                        self.boundaryBoxViz[i].setData(color=pg.glColor('r'))
                        self.bottomSquare[i].setData(color=pg.glColor('r'))
                    else: 
                        self.boundaryBoxViz[i].setData(color=pg.glColor('y'))
                        self.bottomSquare[i].setData(color=pg.glColor('y'))
                else:
                    self.boundaryBoxViz[i].setData(color=pg.glColor('g'))
                    self.bottomSquare[i].setData(color=pg.glColor('g'))
        fallDetEn = 0
        indicesIn = []
