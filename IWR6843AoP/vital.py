...
from gui_threads import (parseUartThread, sendCommandThread, update2DQTGraphThread, updateQTTargetThread3D,
                        updateHeightGraphs, updateVSHeightGraphs, zeroHeightGraphs) #@@

from graphUtilities import (getSphereVertexes, getSphereTriangles, getSphereMesh, 
                            getBoxVertices, getBoxLinesFromVerts, getBoxLines, getBoxLinesCoords,
                            getSquareLines, rotX) #@@
compileGui = 0
#only when compiling
#if (compileGui):
...
class Window(QDialog):
    def __init__(self, parent=None, size=[]):
        ...
        self.yzFlip = 0
        self.frameTime = 120 # frame 처리 Timer interval 
        self.graphFin = 1
        self.hGraphFin = 1
        self.threeD = 1
        self.lastFramePoints = np.zeros((5,1))
        self.plotTargets = 1
        self.frameNum = 0 # frame Count from powerup/start 
        self.lastTID = []
...
#
    # 기본 설정 : uart port, demo category config 
    def setConnectionLayout(self):
        self.comBox = QGroupBox('Connect to Com Ports')
        self.uartCom = QLineEdit('3')    #deb_gp
        self.dataCom = QLineEdit('4')    #deb_gp
    ...
    def setPlotControlLayout(self):
        ...
        #configuration parameter : buttons and tables 
    ...
    def setBoxControlLayout(self, name):
        ...
        boxConLayout.addWidget(tzL,2,2,1,1) # widget row1, col1, row_span, col_span 
    ...
    def updateGraph(self, parsedData):
        ...
        if (self.hGraphFin and (numTargets > 0)):
            ...
            self.heightThread = updateVSHeightGraphs(VS, self.heightPlots, self.frameNum,ADCRAW)
